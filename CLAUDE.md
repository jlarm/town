...
This is a Laravel 12 application with Vue 3 + TypeScript frontend using Inertia.js for seamless client-server communication. The project also integrates Filament for an admin panel interface.
...
- **Database:** SQLite (development), configured for other databases via .env
...
composer dev:ssr
```
Runs the same stack but with Inertia SSR instead of Vite hot reload.
...
npm run build:ssr      # Client + server-side rendering
```
...
npm run lint                   # Fix JavaScript/TypeScript/Vue linting
...
php artisan make:*             # Generate boilerplate (controller, model, etc.)
```

## Architecture & Key Concepts

### Inertia.js Integration
- **Server-side routing** in `routes/web.php` renders Inertia responses
- **Vue pages** live in `resources/js/pages/` and are automatically resolved
- Shared data and props are passed from Laravel controllers to Vue components
- No API layer needed—Inertia handles data transfer transparently

### Wayfinder for Type-Safe Routing
- Laravel Wayfinder generates TypeScript route definitions from Laravel routes
- Auto-generated route helpers available via `resources/js/routes/`
- Route parameters are type-checked at compile time
- Usage: `route('home')` in Vue components with full IntelliSense
...
- `resources/js/wayfinder/` — Generated and custom routing utilities

### Backend Structure
- Standard Laravel structure with minor additions:
  - `app/Filament/` — Filament resources, pages, widgets (auto-discovered)
  - `app/Providers/Filament/AdminPanelProvider.php` — Admin panel configuration

### Testing
- Uses **Pest** (not PHPUnit directly)
- Feature tests in `tests/Feature/` use `RefreshDatabase` trait automatically
- Unit tests in `tests/Unit/`
- Custom test helpers can be added to `tests/Pest.php`
...
### Adding a New Page
1. Create route in `routes/web.php`: `Route::get('/example', fn() => Inertia::render('Example'))`
2. Create Vue component at `resources/js/pages/Example.vue`
3. Wayfinder will auto-generate type-safe route helper

### Creating Filament Resources
```bash
php artisan make:filament-resource ModelName
```
The resource will be auto-discovered in `app/Filament/Resources/`.

### Working with Inertia Forms
Use Inertia's form helper for automatic error handling and loading states:
```typescript
